...
const MIGRATIONS: Migration[] = [
    {
        ...
        statements: [
            `DROP TABLE IF EXISTS card_statement_transactions;`,
...
async function ensureMigrationsTable(database: SQLiteDatabase) {
...
export function useDatabase(): DatabaseHelpers {
    ...
    if (!migrationPromiseRef.current) {
        migrationPromiseRef.current = applyMigrations(database)
    }

    const ready = migrationPromiseRef.current
